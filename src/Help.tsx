...
export function Help({ section = 'usage' }: { section?: 'usage' | 'install' | 'calendar' }) {
  ...
  return <div className="stack">
    ...
    <details open><summary>내 휴무조 고르기 · 바꾸기</summary>
      <p>처음에는 범위 설명을 보고 A/B/C/D 휴무조를 직접 선택하고 확인하세요. 실제 근무조 번호만으로 자동 판정하지 않아요.</p>
      <p>휴무조를 옮기면 설정에서 새 휴무조와 ‘적용 시작일’을 정하세요. 그 날짜부터 새 조의 기존 순환을 따르고 과거는 유지돼요. 시작일이 새 조의 첫 휴무일이 되는 것은 아니에요. 조 변경 이력도 수정·취소할 수 있어요.</p>
    </details>
    <details><summary>달력 이동 · 휴무 계산</summary>
      <p>좌우 화살표로 달을 이동하세요. 가운데 연도·월을 눌러 먼 날짜도 바로 고를 수 있어요. 오늘은 테두리와 글자로 구분해요.</p>
      <p>A → B → C → D 순서로 이틀씩 쉬며 각 휴무조는 8일마다 반복돼요. 캡처에서 확인한 2025년 12월 2일을 A 첫 휴무로 계산해요. 현재도 같은 순환임을 2026년 9월 19일 확인했어요. 기본 근무 표시는 실제 배정 확정을 뜻하지 않아요.</p>
    </details>
    <details><summary>양도 · 교환 · 대바 · 메모</summary>
      <p>날짜를 눌러 원래 일정과 변경 후 실제 일정을 확인하세요. 양도는 ‘그날 나는 근무 / 휴무’를 직접 선택하고 상대방과 메모를 적어요. 대바는 다른 사람 대신 근무하는 날이며 상대방 이름을 적을 수 있어요.</p>
