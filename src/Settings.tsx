...
import { ArrowRight, Download, Upload, BookOpen, Share2, Moon, Sun, Monitor, Pencil, Trash2 } from 'lucide-react';
import type { AppData, Group, GroupChange, Theme } from './types';
import { GROUPS, GROUP_NAMES, GROUP_LABELS, BUILD_ID, ANCHOR, ANCHOR_CONFIRMED_ON } from './config';
...
type Props = { data: AppData; onSave: (data: AppData) => Promise<void>; onRestore: (data: AppData) => Promise<void>; onClose: () => void; installState: InstallState; onInstall: () => void; onPage: (page: 'help' | 'share') => void };
export default function Settings({ data, onSave, onRestore, onClose, onPage, installState, onInstall }: Props) {
  ...
  const close = () => { if (!busy && (!draft || window.confirm('근무 유형 변경 입력을 닫을까요?'))) onClose(); };
  ...
  function goPage(page: 'help' | 'share') { if (!busy && (!draft || window.confirm('저장하지 않은 근무 유형 변경 입력이 있어요. 안내로 이동할까요?'))) onPage(page); }
  ...
  return <Modal title="설정" onClose={close}>
    <div className="stack settings">
      ...
      <section><span className="eyebrow">MY GROUP</span><h3>현재 근무 유형</h3><div className="group-current"><strong>{GROUP_NAMES[groupOn(data, todayKorea())]}</strong><span>{GROUP_LABELS[groupOn(data, todayKorea())]}</span></div><p className="muted">처음 선택한 {GROUP_NAMES[data.initialGroup]}을 기본으로, 적용일마다 근무 유형이 바뀝니다. 초기 선택 이전의 실제 소속을 뜻하지 않습니다.</p>
        {!draft && <button className="button full" onClick={() => startGroup()}>근무 유형 변경 <ArrowRight size={18} /></button>}
        {draft && <div className="stack inset"><h4>{data.groupChanges.some(c => c.id === draft.id) ? '근무 유형 변경 이력 수정' : '새 근무 유형과 시작일'}</h4><label>새 근무 유형<select aria-label="새 근무 유형" value={group} onChange={e => { setGroup(e.target.value as Group); setPreview(null); }}>{GROUPS.map(g => <option key={g} value={g}>{GROUP_NAMES[g]} · {GROUP_LABELS[g]}</option>)}</select></label><label>적용 시작일<input type="date" min="1900-01-01" max="2199-12-31" value={date} onChange={e => { setDate(e.target.value); setPreview(null); }} /></label>
          {!preview ? <div className="row"><button className="button" onClick={() => setDraft(null)}>취소</button><button className="button primary" onClick={previewGroup}>변경 내용 확인</button></div> : <><p className="notice">{date}부터 {GROUP_NAMES[group]}의 일정 규칙을 적용합니다. 다음 근무 유형 변경일까지 영향을 주며, 과거 일정과 대바·양도·교환·메모·휴무 계획은 그대로 남습니다.</p>{affected.length > 0 && <div className="notice"><b>기존 기록 {affected.length}일의 원래 일정이 달라집니다.</b><ul>{affected.map(d => <li key={d}>{d} · 실제 {dayInfo(preview, d).state === 'off' ? '휴무' : '근무'} 기록 유지</li>)}</ul></div>}<button className="button primary" disabled={busy} onClick={() => save(preview)}>확인하고 저장</button></>}
        </div>}
        {data.groupChanges.length > 0 && <div className="history"><h4>근무 유형 변경 이력</h4>{[...data.groupChanges].sort((a, b) => a.date.localeCompare(b.date)).map(c => <div className="history-row" key={c.id}><div><strong>{GROUP_NAMES[c.group]}</strong><span>{c.date}부터 {c.date > todayKorea() ? '· 예정' : '· 적용'}</span></div><button className="icon-button" aria-label={`${c.date} 근무 유형 변경 수정`} onClick={() => startGroup(c)}><Pencil size={18} /></button><button className="icon-button danger" aria-label={`${c.date} 근무 유형 변경 취소`} onClick={() => { if (window.confirm(`${c.date}부터 다음 변경일 전까지 이전 근무 유형의 일정으로 돌아갑니다. 개별 변경과 메모는 유지됩니다. 취소할까요?`)) { try { const next = removeGroupChange(data, c.id); const impacted = Object.keys(data.changes).filter(d => dayInfo(data, d).baseState !== dayInfo(next, d).baseState); if (impacted.length && !window.confirm(`원래 일정이 바뀌는 기존 기록: ${impacted.join(', ')}. 실제 변경 기록은 유지됩니다. 계속할까요?`)) return; void save(next); } catch (e) { setError((e as Error).message); } } }}><Trash2 size={18} /></button></div>)}</div>}
      </section>
      ...
      <InstallPanel state={installState} today={todayKorea()} onInstall={onInstall} />
      <section className="stack small-gap"><button className="menu-button" onClick={() => goPage('share')}><Share2 size={21} /><span>앱 공유 / QR코드</span><ArrowRight size={18} /></button><button className="menu-button" onClick={() => goPage('help')}><BookOpen size={21} /><span>사용설명서</span><ArrowRight size={18} /></button></section>
      <section className="data-management"><h3>데이터 관리</h3><p>설정과 기록은 이 기기의 브라우저에 저장됩니다. 휴대폰을 바꾸거나 사이트 데이터를 지우기 전 백업해주세요.</p><div className="row"><button className="button flex" onClick={() => { try { downloadFile(makeBackup(data), `드래곤휴무-${todayKorea()}.backup.json`, 'application/json'); setNotice('백업 파일을 만들었습니다. 파일이 저장됐는지 확인해주세요.'); } catch (e) { setError((e as Error).message); } }}><Download size={19} />백업하기</button><label className="button flex file-button"><Upload size={19} />백업 불러오기<input type="file" accept=".json,application/json" onChange={async e => { setError(''); const file = e.target.files?.[0]; if (!file) return; setBusy(true); try { if (file.size > 5_000_000) throw new Error('백업 파일이 너무 큽니다. 5MB 이하 파일을 선택해주세요.'); setRestore(parseBackup(await file.text())); } catch (error) { setError((error as Error).message); } finally { e.target.value = ''; setBusy(false); } }} /></label></div>
        {restore && <div className="stack inset"><h4>백업을 확인했습니다</h4><p>{GROUP_NAMES[restore.initialGroup]} 기본 유형 · 변경 일정 {Object.keys(restore.changes).length}개 · 메모 {Object.keys(restore.notes).length}개 · 유형 변경 {restore.groupChanges.length}개</p><p className="notice">이 기기의 현재 기록을 백업 내용으로 대체합니다. 복원 직전 기록은 기기에 안전 복사본으로 남깁니다. 외부 백업도 보관해주세요.</p><div className="row"><button className="button" onClick={() => setRestore(null)}>취소</button><button className="button primary" disabled={busy} onClick={async () => { setBusy(true); setError(''); try { await onRestore(restore); setRestore(null); setNotice('복원했습니다. 달력에서 내용을 확인해주세요.'); } catch (e) { setError((e as Error).message); } finally { setBusy(false); } }}>현재 기록 대체</button></div></div>}
        ...
        <p className="muted">휴대폰과 PC의 기록은 자동으로 동기화되지 않습니다.</p>
      </section>
